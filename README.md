...
El sistema utiliza **MySQL** (vía XAMPP). La aplicación crea 5 bases de datos separadas (`hopecare_clinica`, `hopecare_dashboard`, `hopecare_facturacion`, `hopecare_laboratorio`, `hopecare_medicamentos`) al ejecutar `hopecare_mysql_complete.sql`. Los datos de prueba se insertan mediante `CargarDatosPrueba.java`.
...
**Resetear la BD:** Ejecutar la aplicación con `DROP DATABASE IF EXISTS ...` o borrar manualmente las bases desde MySQL Workbench.
