...
public class LaboratorioController {
    ...
    @FXML private TableColumn<SolicitudExamen, Void> colAccionesSolicitudes;
    ...
    private void configurarColumnasSolicitudes() {
        ...
        colAccionesSolicitudes.setCellFactory(col -> new TableCell<SolicitudExamen, Void>() {
            private final Button btnFill = new Button();
            private final Button btnCancel = new Button();
            private final javafx.scene.layout.HBox container = new javafx.scene.layout.HBox(8, btnFill, btnCancel);

            {
                container.setAlignment(javafx.geometry.Pos.CENTER);

                btnFill.getStyleClass().add("button-action-edit");
                javafx.scene.shape.SVGPath fillIcon = new javafx.scene.shape.SVGPath();
                fillIcon.setContent("M11 2h3v3L5 14l-3 1 1-3L11 2z");
                fillIcon.getStyleClass().add("svg-icon");
                btnFill.setGraphic(fillIcon);
                btnFill.setTooltip(new Tooltip("Completar Resultado"));
                ...
                btnCancel.setTooltip(new Tooltip("Cancelar Solicitud"));

                btnFill.setOnAction(e -> {
                    SolicitudExamen s = getTableView().getItems().get(getIndex());
                    if (s != null) {
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty || getTableRow().getItem() == null) {
                    ...
                    SolicitudExamen s = getTableRow().getItem();
        ...
        dialog.setResultConverter(btn -> {
            if (btn == ButtonType.OK) {
                ...
                try {
                    ...
                    ExamenLaboratorio exam = new ExamenLaboratorio();
                    exam.setNombreExamen(nombre);
                    exam.setDescripcion(description);
                    exam.setPrecio(precio);
                    exam.setTiempoResultadoHoras(tiempo);
                    return exam;
                } catch (NumberFormatException e) {
    ...
    private void cancelarSolicitud(SolicitudExamen selected) {
